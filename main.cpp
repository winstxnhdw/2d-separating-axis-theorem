...
std::vector<Vector2> calculate_bounds(const double x, const double y, const double yaw, const double centre_to_front, const double centre_to_rear, const double centre_to_side) {
    ...
    front_right_point.x = x + centre_to_side * sin(yaw) + centre_to_front * cos(yaw);
    front_right_point.y = y - centre_to_side * cos(yaw) + centre_to_front * sin(yaw);

    Vector2 front_left_point;
    front_left_point.x = x - centre_to_side * sin(yaw) + centre_to_front * cos(yaw);
    front_left_point.y = y + centre_to_side * cos(yaw) + centre_to_front * sin(yaw);
    ...
    rear_right_point.x = x + centre_to_side * sin(yaw) - centre_to_rear * cos(yaw);
    rear_right_point.y = y - centre_to_side * cos(yaw) - centre_to_rear * sin(yaw);
